...
class TaskPlanningNode(Node):
    ...
    def _declare_selection_parameters(self):
        ...
        for field_name, field_info in self.selection_config.items():
            # ignore_override=True so saved launch-param values (e.g. region=2)
            # are accepted even though the range is [0,0] at declaration time.
            # update_selection() clamps the value once viewpoints are loaded.
